...
system('cls')   # clears previous lines before the game

game_data = open('SAVED DATA.txt', 'w')
game_data.write("JOHN'S QUEST V2 SAVED DATA FROM ONE PLAY:\n\n")
...
print('\n<Do you wish to play the FULL STORY MODE?>         yes or no?')  # to either skip the narrator's guidence or not

start_reply = input('[YOU] ')
while str.lower(start_reply) != 'yes' and str.lower(start_reply) != 'no':
    system('cls')
    start_art()
    print('<Do you wish to play the FULL STORY MODE?>         yes or no?')
    start_reply = input('[YOU] ')

game_data.write('\nFULL STORY MODE SELECTED: ')
game_data.write(start_reply)
...
system('cls')   # The part where you get into action
